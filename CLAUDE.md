...
- **Artifacts:** `ArtifactsPage` (list/create) and `ArtifactEditorPage` (edit, preview,
  ...
  **Inline images (GitHub-style):** in `ArtifactEditorPage` you can paste, drag-drop, or
  attach (📎 "Image") an image into the body; it uploads to the **`artifact-images`
  bucket** (migration 0059) and a markdown `![](url)` link is spliced in at the caret.
  Objects live under `‹owner›/‹uuid›/‹name›`; writes are folder-scoped. The bucket started
  public, but **migration 0073 made it private** (issue #157 — an image inside a Private
  artifact was readable by anyone with the URL): reads now go through an RLS policy that
  allows the owner, or anyone when the image's `metadata.artifact_id` names a
  public/unlisted artifact. ⚠️ The editor still splices in a **7-day signed URL**
  (`ArtifactEditorPage.insertImages`), so an embedded image stops loading a week after it
  is pasted. 0073's own note ("frontend switches between signed URLs and public URLs")
  describes the intended fix, which is not implemented — treat this as an open bug, not a
  design. Pure logic (image detection, filename sanitizing, cursor-aware markdown
  insertion) lives in `src/lib/artifactImages.ts` and is unit-tested; the resilient upload
  reuses `uploadPickedFile(path, file, bucket, metadata)`.
...
- **Terminology (glossary, migration 0070):** a workspace glossary so agents understand
  the business's own language. A `terminology` row is `term` + `definition` + `notes` +
  `source` provenance + visibility, and `TerminologyPage` (route `/terminology`, Assets;
  `/terminology/:termId` deep-links one) is the CRUD. Visibility mirrors links/todos:
  `private` (owner + admins) or `workspace` (every member can see **and** edit), RLS
  enforced. `collection_terminology` is the many-to-many join (exact mirror of
  `collection_links`, visibility-inherited RLS), so terms file into collections and the
  generic `collection.item_added` trigger learns them. Seeded `is_builtin` tools
  `create_term` / `list_terms` / `update_term` / `delete_term` / `add_term_to_collection`
  (in `_shared/builtins.ts`) let chat/agents manage the glossary. **Note:**
  `loadCollectionsContext` does NOT yet inject a collection's terms the way it injects
  artifacts/files/to-dos/links/whiteboards/boards — the assistant has to call `list_terms`.
  *(Planned: fold terminology into the collection context block; a REST API + MCP tools.)*
- **Meeting Notes (Granola-style recorder, migration 0095):** `MeetingNotesPage` (route
  `/meeting-notes`, Assets) records a meeting in the browser — mic **plus** tab/app audio
  (Zoom/Meet/Slack) — transcribes it in segments through the **`transcribe` edge function**
  (`verify_jwt: true`), and saves the finished record as an **artifact** (transcript +
  ideas + chat + a generated TLDR/follow-ups) rather than a table of its own. ⚠️
  `transcribe` is the one model call that does **not** go through OpenRouter: it POSTs to
  OpenAI's Whisper endpoint with an `OPENAI_KEY` read from the **Secrets Vault** via
  `read_vault_secret` (an `OPENAI_KEY`/`OPENAI_API_KEY` edge secret is the fallback; the
  page warns up front when neither is set), so it is not covered by `model_profiles`,
  `resolveModel`, or `usage_events`. A live **`MeetingChatPanel`** streams against the transcript while the
  meeting runs, and an ambient monitor checks the latest transcript for ideas every 60s.
  `conversations.meeting_id` (a client-generated uuid stamped at record start, indexed like
  `conversations.card_board_id` in 0076) correlates that live thread with the artifact saved
  at the end, which carries the same id in its `data`. Saving fires a dedicated
  **`meeting.recorded`** event (`emit_meeting_event` trigger, keyed on
  `data.meeting_notes = 'true'`) alongside the generic `artifact.created`, so a Listener can
  target meetings specifically. An in-progress meeting is stashed in `localStorage`
  (`meetingNotes:active`) so a reload doesn't lose the transcript.
...
- **PDF knowledge (RAG):** uploading a PDF enqueues a `documents` row (trigger on
  ...
  **workspace-shared by default** (`documents.scope = 'workspace'`): any member's
  ...
  owner can change scope. PDFs are not the only door: `add_note` pushes raw text
  (e.g. meeting notes) into the same `documents` table via the shared
  `_shared/knowledge.ts` (`source in ('pdf','note')`, migration 0024), so adding a
  front door means wiring it to that helper, not re-implementing chunk→embed→store.
  Scope is **separate** from `files.visibility` — the raw PDF blob stays
  owner-private; only the text chunks are shared. Text-layer PDFs only for now
  (scanned → Stage 2 vision).
  **Hybrid retrieval (migrations 0086 + 0087):** the seeded built-in
  `search_documents` no longer runs the cosine-only `match_document_chunks`. One
  `search_chunks_hybrid` RPC returns a candidate pool carrying each chunk's
  **vector rank** and its **keyword (full-text) rank**, and the pure, unit-tested
  `_shared/retrieval.ts` fuses the two orderings with **reciprocal rank fusion**
  (`RRF_K = 60`) — vector search alone misses exact terms/IDs/rare tokens, keyword
  search alone misses paraphrases. 0086 shipped this as two RPCs fused in the edge
  function; 0087 collapsed it to one round trip because the `rag` eval's three
  retrieval calls per case were tipping the edge worker over its budget (the two
  0086 RPCs are left in place, unused). The RPC is scope-aware
  (`workspace ∪ caller's private`) and service-role only, and returns
  `document_updated_at` so `citationLabel()` can cite recency
  ("PII Program.pdf · updated 2026-06"). An empty result returns an explicit
  "nothing indexed on this topic" string so the model says so instead of guessing.
  The same helper backs the `rag` eval target, so live search and the eval can't
  drift.
...
- **Agent runs (observability, migration 0108):** the loops used to persist almost nothing
  per step — a tool call logged only its NAME to `activity_log`, `usage_events` had no run
  grouping, and the in-memory trace was thrown away — so you couldn't troubleshoot what an
  unattended agent actually did. `agent_runs` is one row per invocation (surface, status,
  model, input, final output, turns, tool_calls, total_tokens, cost, timings);
  `agent_run_steps` is one row per step — a model turn or a tool call **with its raw input
  and output** — deliberately rows, not a jsonb blob, so a builder can filter ("runs that
  called `send_email`", "runs with a tool error"). `RunRecorder`
  (`_shared/run_recorder.ts`, pure helpers in `run_recorder_pure.ts`) is wired into the
  four agent loops (chat, scheduler, webhook, slack) at the seam they already share, and is
  best-effort like `recordUsage`/`logActivity` — a failure here never breaks a run.
  `RunRecorder.open` returns **null when there's no agent to attribute the run to**, so
  plain user chat creates no rows. `AgentDetailPage` (route `/agents/:id`) renders the runs
  + step trace; the pure formatting lives in `src/lib/agentRuns.ts` (unit-tested). Surfaces:
  `chat | schedule | webhook | slack | listener | manual`. RLS is owner-or-admin (step
  payloads can hold a webhook body or someone's chat, so it is deliberately NOT
  workspace-wide) with **no INSERT/UPDATE policies at all** — the loops write as the
  service role. This shipped as a colliding `0107` and was renumbered to `0108`.
...
- **Email:** two seeded `is_builtin` tools — `send_email` and `check_email` — let any
  ...
  Sending goes through an HTTP provider (Postmark / Resend, not raw SMTP). Receiving has
  **two doors, both landing in `inbox_messages`** (so `check_email` reads one table either
  way): **push** — a provider's inbound-parse POSTs each mail to the public `email-inbound`
  edge function (`verify_jwt: false`, token-gated like `webhook`), which normalizes it; and
  **pull** — an IMAP mailbox registered under *Inbox → Inboxes* (see the IMAP entry below).
  **Credentials live only in Supabase Vault:** the non-secret config sits in
  ...
  not just chat. Email-triggered agents are no longer a follow-up: every inbound row emits
  `message.received` (0063/0064), so a Listener runs an agent per message.
- **IMAP inboxes (pull email, migrations 0102–0105):** `email_accounts` registers a mailbox
  the workspace **polls** — the counterpart to provider push. `InboxAccountsPage`
  (`/inbox/accounts`, reached from Inbox → *Inboxes*) is the CRUD; the mailbox **password
  lives only in Supabase Vault** (`secret_id` pointer, same pattern as the email key and
  vault secrets), writes go through security-definer RPCs, and the decrypted password is
  read only by the service role (`read_email_account_secret`). Visibility mirrors
  links/todos: `private` or `workspace` (ingested mail inherits the account's visibility).
  The **`email-poll` edge function** (`verify_jwt: false`, cron-secret gated, in
  `_automation_cron_jobs()` so every tenant self-schedules it) opens IMAP over
  `Deno.connectTls`, fetches UIDs above `last_seen_uid` (a fresh inbox backfills only the
  ~10 most recent), and writes `inbox_messages` (`source='email'`) — which emits
  `message.received`, so a Listener routes it exactly like a webhook. Each account is
  polled at most once per its `poll_interval_minutes`; per-tick and per-account caps bound
  the work; any failure lands on `last_error` (badge on the card) instead of throwing the
  tick. **Dedupe is a plain insert + `23505` catch**, NOT `upsert(onConflict)` — the unique
  index on `(source, external_id)` is *partial* (`where external_id is not null`) and
  Postgres can't infer a partial index for `ON CONFLICT`, so the original upsert silently
  failed on every row while advancing the cursor (0103's fix). `mark_seen` (0103) flags
  imported mail `\Seen` on the server; `reset_email_account_cursor` powers **Re-scan**.
  RFC822 parsing is the pure, unit-tested `_shared/imap.ts`; the socket/framing client is
  `_shared/imap_client.ts`, shared with the **`imap-test`** function (`verify_jwt: true`)
  behind the editor's *Test connection* button. Routing (0105) is not a parallel engine:
  the editor manages ONE per-inbox `event_listener` (scoped by `match.account_id`, 0104)
  linked from `email_accounts.routing_listener_id`, so "save to a table / run an agent /
  run a function" runs through `event-dispatch` like every other rule. Setup guide
  (app passwords, per-provider host/port): `docs/events-and-inbox.md`. *(Planned:
  Google/Microsoft OAuth connectors on the same table; STARTTLS on port 143.)*
